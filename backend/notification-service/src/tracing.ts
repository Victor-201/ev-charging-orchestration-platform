 ...
 *   OTEL_ENABLED=true           - Enable tracing (default false during dev)
...
// Only initialize when explicitly enabled - avoids overhead during dev/test
  ...
  void (async () => {
    try {
      ...
      const sdk = new NodeSDK({
        // Use string keys directly to ensure compatibility across all OTel versions
        serviceName,
      ...
      // Flush and shutdown when the process exits (Graceful Shutdown)
      ...
      // Do not crash the app if OTel encounters an error
