 ...
 *
 * Implementation thực sử dụng OLS Linear Regression (regr_slope/intercept/r²)
 * KHÔNG phải EWA. Test này phản ánh đúng schema SQL trả về.
 *
 * Mock DataSource.query() trả về rows với schema:
...
/** OLS regression row — schema trả về từ PostgreSQL regr_ functions */
function makeOlsRow(
  hour: number,
  slope: number,
  intercept: number,
...
describe('PeakHourDetector', () => {
  ...
  describe('detectForStation()', () => {
    ...
    it('row có avgSessions cao nhất → rank=1, isPeak=true', async () => {
      ...
      expect(result[3].isPeak).toBe(false);  // không phải top 3
    });

    it('peakScore của rank-1 = 1.0 (normalize bằng max)', async () => {
      const rows = [
    ...
    it('parse integer hourOfDay từ string', async () => {
    ...
    it('peakScore = 0 khi maxSessions = 0 (empty data guard)', async () => {
      // Trường hợp avg_sessions = 0 cho tất cả rows
  ...
  describe('forecastNextDay() — OLS Linear Regression', () => {
    ...
    it('trả về empty array nếu không có regression data', async () => {
      const detector = makePeakDetector([[]]);
      const result   = await detector.forecastNextDay('station-001');
      expect(result).toEqual([]);
    });

    it('tính forecastSessions đúng bằng OLS: ŷ = intercept + slope * 29', async () => {
    ...
    it('forecastSessions >= 0 ngay cả khi OLS cho kết quả âm (floor = 0)', async () => {
      // slope=-2, intercept=0 → ŷ = -58 → phải clamp về 0
    ...
    it('trend = increasing khi slope > 0', async () => {
      const rows     = [makeOlsRow(18, 0.5, 10, 0.7)];  // dương slope
      const detector = makePeakDetector([rows]);
      const result   = await detector.forecastNextDay('s-001');
      expect(result[0].trend).toBe('increasing');
    });

    it('trend = decreasing khi slope < 0', async () => {
      const rows     = [makeOlsRow(8, -0.3, 15, 0.5)]; // âm slope
      const detector = makePeakDetector([rows]);
      const result   = await detector.forecastNextDay('s-001');
      expect(result[0].trend).toBe('decreasing');
    });

    it('trend = stable khi slope ≈ 0', async () => {
    ...
    it('confidence thấp khi dataPoints < 14 (data penalty)', async () => {
      // dataPoints=7 → penalty=0.5 → confidence bị giảm
    ...
    it('confidence cao khi dataPoints >= 14 và r² cao', async () => {
