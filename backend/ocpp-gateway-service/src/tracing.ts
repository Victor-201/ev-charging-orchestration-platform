...
// Only start when explicitly enabled - avoids overhead during dev/test
  ...
  void (async () => {
    try {
      ...
      const sdk = new NodeSDK({
        // Use string keys directly for compatibility with all OTel versions
        serviceName,
        ...
        instrumentations: [
          getNodeAutoInstrumentations({
            '@opentelemetry/instrumentation-http': {
              // Ignore health check to reduce noise
      ...
      // Flush and shutdown when process exits (Graceful Shutdown)
