...
## 1. EVOLTBOARD - TRANG QUẢN TRỊ TRUNG TÂM (WEB ADMIN)
...
## 2. EVOLTTOUCH - MÀN HÌNH TRỤ SẠC (KIOSK SMART DISPLAY)
...
## 3. EVOLTSYNC - ỨNG DỤNG DI ĐỘNG (MOBILE APP)
...
- **Thông báo đẩy (Push Notification):** Tích hợp dịch vụ đám mây **Firebase Cloud Messaging (FCM)**. Khi luồng xử lý bất đồng bộ từ hệ thống Backend hoàn tất (ví dụ: _Kết thúc phiên sạc_, _Đến lượt sạc trong hàng đợi_, _Cập nhật công nợ_), Backend sẽ bắn tín hiệu qua FCM để đẩy thông báo Real-time trực tiếp lên màn hình thiết bị của người dùng, giúp nắm bắt trạng thái tức thời mà không cần mở app.
...
| :------------------ | :----------------------------- | :----------------- | :------------------------------- | :--------------------------------- |
| **EVOLTBOARD** (Admin)  | Quản trị tổng thể, Analytics   | **Next.js (TS)**   | Leaflet + OSM                    | Vercel / Netlify                   |
| **EVOLTTOUCH** (Kiosk)  | Hiển thị SoC, QR Thanh toán    | **React + Vite**   | N/A                              | Local Device (Raspberry Pi/Tablet) |
| **EVOLTSYNC** (Mobile)   | Đặt chỗ, Chỉ đường, Ví điện tử | **Flutter (Dart)** | `flutter_map` + OpenRouteService | .APK / App Store                   |
